  ...
  <div class="production-tab">
    ...
    <div class="summary-bar">
      <a-row :gutter="[16, 12]">
        ...
        <a-col :xs="24" :sm="6">
          <div class="summary-item">
            <span class="summary-label">所属时区</span>
            <span class="summary-value">
              <a-tag color="blue" :bordered="false" class="m-0">{{ regionName }}</a-tag>
            </span>
          </div>
        </a-col>
        <a-col :xs="24" :sm="6">
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'startTime'">
          <div class="flex items-center gap-1.5">
            ...
            <span v-if="record.startTime" class="text-[11px] text-gray-400 bg-gray-100 px-1 rounded">{{ regionName }}</span>
          </div>
        </template>
        <template v-else-if="column.key === 'endTime'">
          <div class="flex items-center gap-1.5">
            <span class="text-gray-500 font-mono text-[13px]">{{ record.endTime ? formatDateTime(record.endTime) : '-' }}</span>
            <span v-if="record.endTime" class="text-[11px] text-gray-400 bg-gray-100 px-1 rounded">{{ regionName }}</span>
          </div>
        </template>
...
import { getProductionInfo } from '@/services/DataMap/tableService.js'

const props = defineProps({
  tableId: { type: String, default: '' },
})

const productionInfo = ref({ pipelineName: '', scheduleInterval: '', region: 'ID', lastRunAt: null, executions: [] })

watch(() => props.tableId, async (id) => {
  if (!id) return
  const data = await getProductionInfo(id)
  if (data) {
    // 模拟增加一个 region 字段，实际应由后端返回
    data.region = ['ID', 'CN', 'MX', 'ES'][Math.floor(Math.random() * 4)]
    productionInfo.value = data
  }
}, { immediate: true })
...
const regionName = computed(() => {
  return regionMap[productionInfo.value.region] || '当地时间'
})
...
const execColumns = [
  ...
  { title: '实例名称', dataIndex: 'taskName', key: 'taskName', width: 280, ellipsis: true },
