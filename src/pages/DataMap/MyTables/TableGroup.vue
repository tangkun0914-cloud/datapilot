  ...
  <div>
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.dataIndex === 'timeField'">
          <span class="text-xs text-gray-500 font-mono">{{ formatTime(record[timeType] || record.updatedAt) }}</span>
        </template>
      </template>
    </a-table>
...
const props = defineProps({
  ...
  timeType: {
    type: String,
    default: 'updatedAt', // 可选值: 'updatedAt' (更新时间), 'favoritedAt' (收藏时间), 'viewedAt' (浏览时间)
  },
})
...
const timeColumnTitle = computed(() => {
  if (props.timeType === 'favoritedAt') return '收藏时间'
  if (props.timeType === 'viewedAt') return '浏览时间'
  return '更新时间'
})
...
const columns = computed(() => [
  ...
  { 
    ...
    width: '15%',
    sorter: (a, b) => {
      const timeA = a[props.timeType] || a.updatedAt || 0
      const timeB = b[props.timeType] || b.updatedAt || 0
      return timeA - timeB
    },
