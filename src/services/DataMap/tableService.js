...
export async function getTableByFqn(fqn) {
  if (import.meta.env.VITE_USE_MOCK !== 'false') {
    const { tableDetail } = await import('@/mock/DataMap/detail.js')
    return tableDetail
  }
...
export async function getTableSampleData(id) {
  if (import.meta.env.VITE_USE_MOCK !== 'false') {
    const { sampleData } = await import('@/mock/DataMap/detail.js')
    return sampleData
  }
...
export async function getTableColumnProfile(id) {
  if (import.meta.env.VITE_USE_MOCK !== 'false') {
    const { columnProfiles } = await import('@/mock/DataMap/detail.js')
...
export async function getTableVersions(id) {
  if (import.meta.env.VITE_USE_MOCK !== 'false') {
    const { changeHistory } = await import('@/mock/DataMap/detail.js')
...
export async function getProductionInfo(id) {
  if (import.meta.env.VITE_USE_MOCK !== 'false') {
    const { productionInfo } = await import('@/mock/DataMap/detail.js')
...
export async function getRecentViewed() {
  if (import.meta.env.VITE_USE_MOCK !== 'false') {
    const { recentViewed } = await import('@/mock/DataMap/assets.js')
    return recentViewed
  }
...
export async function getFavorites() {
  if (import.meta.env.VITE_USE_MOCK !== 'false') {
    const { favorites } = await import('@/mock/DataMap/assets.js')
    return favorites
  }
